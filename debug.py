...
class BackpackAuthenticator:
    def __init__(self, private_key):
        ...
        self.signing_key = nacl.signing.SigningKey(private_key, encoder=nacl.encoding.Base64Encoder)
...
def rest_request(method, path, instruction, params=None, is_public=False):
    import json
    logging.debug(f"REST Request → {method} {path}, instruction={instruction}, params={params}")

    headers = {} if is_public else get_headers(instruction, params)

    # For debugging signatures and headers
    logging.debug(f"Headers generated: {json.dumps(headers, indent=2)}")
