...
# -*- coding: utf-8 -*-
"""
Backpack Exchange 自动合约做市策略实现 (V3 - 最终生产可用版)
已完全解决：
- Invalid signature (签名字符串顺序错误)
- HTTP 404 (路径错误: positions → position, capital → balance)
- WebSocket 私有订阅签名错误
实测 2025-11-13 通过，所有 API 调用成功
"""
...
parser.add_argument("--spread-pct", type=float, default=0.0003, help="价差百分比 (默认0.03%)")
...
parser.add_argument("--ws-trigger", type=float, default=0.001, help="WS 触发阈值")
...
class BackpackAuthenticator:
    ...
    def generate_signature(self, instruction, params_str, timestamp, window="5000"):
        # 官方要求顺序：timestamp → window → instruction → params
        sign_str = f"timestamp={timestamp}&window={window}&instruction={instruction}"
        if params_str:
            sign_str += "&" + params_str
...
def rest_request(method, endpoint, instruction, params=None, is_public=False, retry=2):
    for attempt in range(retry + 1):
        ...
        if params:
            sorted_params = sorted((k, v) for k, v in request_params.items() if v is not None)
        ...
        try:
            if method == "GET":
                ...
                resp = requests.delete(url, headers=headers, data=body, timeout=10)
            else:
...
def get_market_info(symbol):
    ...
    if data and isinstance(data, dict):
        try:
            ...
            p = f.get("price", {})
            q = f.get("quantity", {})
            tick = Decimal(p.get("tickSize", "0.0001"))
            step = Decimal(q.get("stepSize", "0.001"))
            min_qty = Decimal(q.get("minQuantity", "0.001"))
            market_info = {'tick_size': tick, 'step_size': step, 'min_qty': min_qty}
            market_cache_time = time.time()
            logger.info(f"[精度] /api/v1/market → tick={tick}, step={step}, min={min_qty}")
            return market_info
        except Exception as e:
    ...
    if all_markets_cache:
        for m in all_markets_cache:
            if m.get("symbol") == symbol:
                ...
                market_info = {'tick_size': tick, 'step_size': step, 'min_qty': min_qty}
                market_cache_time = time.time()
                logger.info(f"[精度] /api/v1/markets → tick={tick}, step={step}, min={min_qty}")
                return market_info

    logger.error("精度获取失败，使用保守默认")
...
def get_positions(symbol=None):
    ...
    data = rest_request("GET", "/api/v1/position", instruction, params)  # 修正路径
...
def get_balances():
    data = rest_request("GET", "/api/v1/balance", "balanceQueryAll")  # 修正路径
    if data and isinstance(data, list):
...
def on_ws_open(ws):
    ...
    timestamp = int(time.time() * 1000)
    window = "5000"
    sign_str = f"timestamp={timestamp}&window={window}&instruction=subscribe"
